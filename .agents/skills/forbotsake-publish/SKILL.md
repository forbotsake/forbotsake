...
description: >
  ...
  (format for copy-paste). Handles X/Twitter threads, LinkedIn posts, blog
  posts,
  ...
  Use when: "publish this", "format for twitter", "post to X", "ship this
  content",
  ...
  Proactively invoke when the user has content in content/ and says they want to
  ship it.
---
...
- Skip the content/platform selection direct conversation with the user
...
3. Ask the user directly in conversation: "I see you're logged in as {handle} on {platform}. Is that the right account to post from?"
