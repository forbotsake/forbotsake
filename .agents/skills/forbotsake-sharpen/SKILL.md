...
description: >
  Stage 4.5: SHARPEN. Takes a specific outreach target (person or organization)
  and

  produces a deep execution plan with contextual research, relationship mapping,

  angle selection, and a multi-touch sequence. Reads your founder profile and
  strategy
...
Ask the user directly in conversation: A) Resume, B) Start fresh.
...
Ask the user directly in conversation. Capture the answer and write a minimal `founder-profile.md`:
...
**Fast path first** (Step 2 of framework). Ask via direct conversation with the user:
...
Ask the user directly in conversation. Wait for confirmation before proceeding.
...
Present to the user. No direct conversation with the user needed unless the user wants to correct.

## Phase 5: Generate Angles (Framework Step 4)

Based on research + position, generate 3 outreach angles. Present via direct conversation with the user:
...
Ask the user directly in conversation for any FAIL-level gate. If user overrides, note it in the plan.
