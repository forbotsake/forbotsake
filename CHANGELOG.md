...
- 4 new knowledge frameworks: visual-strategy, brand-identity, media-providers, visual-narrative.
- `bin/src/satori-card.ts` — local text-card renderer using Satori + resvg.
- `bin/brand-extract` — brand identity extraction helper.
- Content schema v2 with visual fields. Engagement feedback loop in published-log.md.

### Changed
- `/forbotsake-marketing-start` adds Phase 3.5 (Brand Visual Identity).
- `/forbotsake-create` adds visual judgment + generation (Phases 3.5-3.6).
- `/forbotsake-publish` attaches visual assets via `upload_image` MCP tool.
- `/forbotsake-content-check` gains Dimension 8 (Visual Consistency).
- `/forbotsake-content-plan` adds Visual column to calendar.
- `/forbotsake-go` adds Stage 1.5 (Media Providers Setup).

## 0.5.0 (2026-04-05)

### Added
- **`/forbotsake-cron` — autonomous auto-posting via cron.** Content posts itself via `claude -p --chrome`.
- Tick mode, autonomous receipts, scheduled_datetime support, headless safety guard.

### Changed
- CLAUDE.md: Autopilot Mode section. README: Autopilot Mode section.

## 0.4.0 (2026-04-05)

### Added
- **Adversarial review gates** — Strategy Reviewer, Content Red Team, Publish Kill Switch.
- `knowledge/banned-patterns-defaults.md`, `FORBOTSAKE_FAST=1`, gate metrics logging.

### Changed
- Pipeline: CREATE → RED TEAM → REVIEW → KILL SWITCH → SHIP.
- Content-check runs adversarial review before scorecard. Create reads reviewer_notes.
